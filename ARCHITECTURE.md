...
**选择**：可拖动 + 可锁定 + 每应用可配阈值和颜色。

**交互行为**：

| 操作 | 行为 | 说明 |
|------|------|------|
| 单击拖动 | 移动悬浮窗位置 | 默认可拖动，跟随手指/鼠标 |
| 长按 (>800ms) | 切换锁定状态 | 锁定后不可拖动，防止误触 |
| 双击 | 打开 Dashboard | 从悬浮窗快速进入主面板 |
| 右键 (桌面) | 上下文菜单 | 快捷设置、退出悬浮窗 |

**锁定状态指示**：
- 未锁定：悬浮窗四角显示半透明拖动手柄
- 已锁定：手柄消失，仅显示计时内容
- 切换时短暂震动反馈 (Android) / 颜色闪烁 (Windows)
...
具体任务：
├── Task 1.1  初始化 Flutter 项目 + 依赖配置                [ ]
├── Task 1.2  AWClient — REST API 对接                     [✅ 已完成]
├── Task 1.3  TimerService — 事件驱动计时                    [ ]
